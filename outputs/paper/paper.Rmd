...
# Introduction
 ...
 Relatively speaking, the science of statistics is a new discipline. In 1998, the public image of statistics was poor, and almost nobody knows what statisticians do[@fromstats]. Now, statistics is an essential tool for nearly all millennial industries. Accompanied by technological improvements in computers, R has become a necessary tool for all statistical practitioners; that makes the teaching and training of R extremely important.
  ...
  In this report, we run our analysis in `R` [@citeR]. We also use the `tidyverse` package, which was written by [@tydr]. See the complete list of packages used in the appendix.
...
The total sample size is 48, from which 7 respondents answered ‘no’ to the qualification questions. That makes the sample size 41. Figure 1 shows the distribution of the programs from the sample. The colouring at the end of the bars indicates the respondent does not have any coding experience. We notice that this is a small part of the sample. 
...
## MRP
  ...
  We use multilevel regression with post-stratification to predict the anxiety level distribution among our population. This method adjusts our estimation results by first fitting a multilevel regression model using the sample, then applying it to the post-strat data set to predict the population. Specifically, each individual is defined by his/her sex, program, cgpa, and whether he/she has previous experience with coding. For each individual in the census, we predict that person’s anxiety score using the previous model using `add_predict_draws()` from `tidybayes.` Then we aggregate the cell-level estimates to the population level. Using y to represent the anxiety score, we can estimate the anxiety score for any subpopulation:

\begin{equation}
\hat{y}_{sub}^{PS} = \frac{\sum{N_j\hat{y_j}}_{j\in{J_{sub}}}}{\sum{N_j}_{j\in{J_{sub}}}}  
\end{equation}

  We get our post-stratification estimates by equation (1), where $J_s$ are all the cells that are in the subpopulation and  $\hat{y}_{sub}^{PS}$ is based on our multilevel regression model.
You can see that the key to an accurate estimate relies not only on how well the model fits the data but also on the level to which the census represents the population. 
...
res_sex %>% 
  ...
  geom_point(data = clean_data %>% #Add raw data average
               group_by(sex) %>% 
               summarise(total = sum(anxiety_score), n =n()) %>% 
               mutate(avg = total/n),
             aes(sex, avg, color = "Raw Survey Data")
             )+
  theme_light()+
...
res_program %>% 
  ...
  geom_point(data = clean_data %>% 
               ...
               summarise(total = sum(anxiety_score), n =n()) %>% 
               mutate(avg = total/n),
             aes(program, avg, color = "Raw Survey Data")
             )+
  theme_light()+
...
res_cgpa %>% 
  ...
  geom_point(data = clean_data %>% 
               ...
               summarise(total = sum(anxiety_score), n =n()) %>% 
               mutate(avg = total/n),
             aes(cgpa, avg, color = "Raw Survey Data")
             )+
  theme_light()+
...
res_code %>% 
  ...
  geom_point(data = clean_data %>% 
               ...
               summarise(total = sum(anxiety_score), n =n()) %>% 
               mutate(avg = total/n),
             aes(coding_exp, avg, color = "Raw Survey Data")
             )+
  theme_light()+
